...
function linter() {
    ...
    if [[ "${NUM_FILES}" == "0" ]]; then
        log ""
        log "No files to lint based on current lint filter: ${ARGS[*]}"
        NUM_FILES="0"
    else
        log ""
    ...
    # ensure we're starting from root of moxy repo
    cd "${MOXY}" > /dev/null  || ( error "Not able to change into repos directory" && exit 10 )
    ...
    for file in "${FILES[@]}"; do
        CMD="shellcheck --color=always -s bash -o all ${file}"
        result=$(eval "$CMD")
        ...
        if [[ -z "${result}" ]]; then
            ...
            # there are messages so it's a fail
